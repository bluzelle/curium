...
import {getBluzelleClient, serializeRequests, defaultLease} from "./client-helpers";
...
beforeEach(async function () {
    if(getBluzelleClient() === 'remote') {
        this.timeout(50000);
        console.log('--------------------------------------------');
        await axios.post(`http://${PROXY_ADDRESS}:${PROXY_PORT}`, {method: 'deleteAll', args: [defaultLease()]})
    }
});
