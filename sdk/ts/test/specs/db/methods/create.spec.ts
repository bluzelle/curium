...
import {createKeys, DEFAULT_TIMEOUT, sentryWithClient} from "../../../helpers/client-helpers/client-helpers";
...
describe('create()', function () {
    ...
    it('should be able to store an empty value', async () => {
        await bz.create('key', '', defaultLease)
    ...
    it.skip('should handle large documents', async () => {
        ...
        await bz.create('key', json, defaultLease);
    ...
    it('should throw an error if a key already exists and not update the value', async () => {
        await bz.create('myKey', 'firstValue', defaultLease)
            .then(async () => expect(await bz.read('myKey')).to.equal('firstValue'));

        await bz.create('myKey', 'secondValue', defaultLease)
    ...
    it.skip('should throw an error if assigned insufficient max fee', async () => {
        expect(
            await bz.create('myKey', 'myValue', defaultLease).catch(e => e.error)
        ).to.equal('insufficient fees');
    });
    ...
    it('should throw an error if the lease time is less than 0', async () => {
        await expect(bz.create('aKey', 'aValue', {...defaultLease, days: -2})).to.be.rejectedWith('Invalid lease time');
    })
    ...
    it('can handle multiple creates', () =>
        Promise.all([
            ...
            bz.create('foo4', 'bar4', defaultLease),
        ])
            .then(x => x)
            .then(() => Promise.all([
                bz.read('foo1'),
